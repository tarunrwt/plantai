    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem", background: "var(--color-bg)" }}>
      ...
      >
        <div style={{ textAlign: "center", marginBottom: "2rem" }}>
          ...
          <h1 style={{ marginTop: "1.25rem", fontSize: "1.5rem", fontWeight: 700, color: "var(--color-text)" }}>Welcome back</h1>
          <p style={{ marginTop: "0.375rem", color: "var(--color-muted)", fontSize: "0.875rem" }}>Sign in to continue to PlantAI</p>
        </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            <div>
              ...
              <div style={{ position: "relative" }}>
                ...
                <input id="login-email" className="input" type="email" placeholder="you@farm.com" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} required style={{ paddingLeft: "2.25rem" }} />
              </div>
            </div>
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.375rem" }}>
                <label className="label" style={{ marginBottom: 0 }}>Password</label>
                <Link href="/forgot-password" style={{ fontSize: "0.75rem", color: "var(--color-accent)", textDecoration: "none" }}>Forgot password?</Link>
              </div>
              <div style={{ position: "relative" }}>
                <Lock size={16} style={{ position: "absolute", left: "0.75rem", top: "50%", transform: "translateY(-50%)", color: "var(--color-muted)" }} />
                <input id="login-password" className="input" type={showPassword ? "text" : "password"} placeholder="Your password" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} required style={{ paddingLeft: "2.25rem", paddingRight: "2.75rem" }} />
